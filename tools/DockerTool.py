...
class DockerToolInput(ToolInput):
    ...
    files_to_copy: List[str] = ToolField(name='Files to copy',
                                         description='List of file paths to copy to the container. There will be copied to the same path in the container. Example: ["/path/to/file1", "/path/to/file2"]')
...
def start_container(docker_client):
    ...
    container = docker_client.containers.run("python:3.10-slim", name=container_name, detach=True,
