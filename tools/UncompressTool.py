...
def create_output_dir(compressed_file_path, extension=None):
    output_dir = (
        compressed_file_path[: -len(extension)] if extension else compressed_file_path
    )
...
def ungzip(compressed_file_path, extension=None):
    ...
    output_file_path = os.path.join(
        output_dir, os.path.basename(compressed_file_path)[:-3]
    )
...
def unbzip2(compressed_file_path, extension=None):
    ...
    output_dir = create_output_dir(compressed_file_path, extension)
    output_file_path = os.path.join(
        output_dir, os.path.basename(compressed_file_path)[:-4]
    )
...
class UncompressTool(ToolWrapper):
    name: str = "UncompressTool"
    description: str = (
        "This tool uncompresses a file and returns the path list of the uncompressed files."
    ...
    def run(self, input_params: Dict, *args, **kwargs):
        ...
        if not os.path.exists(compressed_file_path):
            return {
                "error": f"The mentioned path was not found {compressed_file_path}."
            }
        ...
        return {
            "uncompressed_files_paths": uncompress_functions[extension](
                compressed_file_path, extension
            )
        }
